...
class _FloatingHeaderState extends State<FloatingHeader>
    with AutomaticKeepAliveClientMixin<FloatingHeader> {
  @override
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_){
        _interceptor = FloatingHeaderInterceptor(callback: (listbadges) {
  ...
  Future<bool> findLocation() async {
    Future<void> retrieveLocation() async {
      try {
        Position position = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.bestForNavigation).timeout(const Duration(seconds: 6));
        final param = {"lat": position.latitude, "lng": position.longitude};
        try {
          Response res = await _dio.get(Endpoint.requestKota, queryParameters: param);
          var city = res.data;
          ...
          var message = e.response == null ? "Silahkan cek internet anda" : GetErrorMessage.getErrorMessage(e.response?.data['errors']);
        ...
        AppDialog.snackBar(text: "Maaf, Koneksi internet anda kurang mendukung, Silahkan coba lagi / Pilih Kota yang tersedia");
      }
    }
                            ...
                            children: <Widget>[
                              ...
                              InkWell(
                                onTap: () {
                                  ...
                                  AppDialog.snackBar(text: "pergi ke SearchView");
                                  // Navigator.push(
                                  //   context,
                                  //   PageRouteBuilder(
...
class FloatingHeaderInterceptor extends InterceptorsWrapper {
  Function(List<String>)? callback;

  FloatingHeaderInterceptor({this.callback});
